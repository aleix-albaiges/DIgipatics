...
Pérdida train vs val (misma función GuidedLoss = CE ponderada + Dice):
    ...
    - Esta variante (training_conchv2_generalization.py): dropout FPN, weight_decay, label_smoothing,
      LR cosine+warmup opcional, ratio encoder/decoder, EMA, augmentations HueSat+Elastic, tracking
      val_loss vs macro-F1. Checkpoints por defecto en checkpoints_conch_masklut_generalization/.

Usage (este fichero):
    python training_conchv2_generalization.py --ema --lr-schedule cosine --fold Val1
"""
...
OUTPUT_DIR = BASE_DIR / "checkpoints_conch_masklut_generalization"
...
# ─────────────────────────────────────────────────────────────────────────────
# Augmentations — base + opcional HueSat + Elastic
# ─────────────────────────────────────────────────────────────────────────────
...
class ModelEMA:
    """Media móvil exponencial de pesos (y buffers) para evaluar/guardar con mejor generalización."""
...
def build_lr_scheduler(optimizer, config: dict, max_epochs: int):
    """ReduceLROnPlateau (métrica macro-F1 en train_fold) o warmup + cosine por época."""
...
def train_fold(fold_name: str, config: dict, device: torch.device, dry_run: bool = False, use_wandb: bool = True):
    ...
    print(
        f"\n  [Resumen {fold_name}] Métrica de checkpoint: macro-F1 (mejor época {best_macro_f1_epoch}, "
        f"F1={best_macro_f1:.4f}). Mejor val_loss en época {best_val_loss_epoch} "
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directorio para checkpoints .pth (por defecto: checkpoints_conch_masklut_generalization).",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="P",
        help="Dropout2d en cabeza FPN (default: 0.1; 0 desactiva).",
    )
    parser.add_argument(
        "--label-smoothing",
        type=float,
    ...
    parser.add_argument(
        ...
        default=None,
        help="plateau=ReduceLROnPlateau sobre macro-F1; cosine=warmup+CosineAnnealing por época.",
    )
    parser.add_argument(
        "--lr-warmup-epochs",
        type=int,
        default=None,
        metavar="W",
        help="Épocas de warmup lineal (solo con --lr-schedule cosine).",
    )
    parser.add_argument(
        "--lr-min",
        type=float,
    ...
    if use_wandb and not args.dry_run:
        try:
            wandb_config = {
                "script": "training_conchv2_generalization",
                "img_size": IMG_SIZE,
