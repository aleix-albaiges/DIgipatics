...
- Mapeo máscara ↔ clases: sicap_mapping (``build_mask_lut``, ``--gg5-gray-min``, ``clean_all_speckles``).
...
from sicap_mapping import (
    DEFAULT_GG5_GRAY_MIN,
    MASK_LUT as _DEFAULT_MASK_LUT,
    build_mask_lut,
    clean_all_speckles,
)
...
def _worker_init_fn(worker_id: int, base_seed: int):
    """Worker init picklable para Windows multiprocessing."""
...
DEFAULT_CONFIG = {
    ...
    "num_workers"   : 4,
    # Regularización adicional para reducir overfitting del decoder
    "weight_decay"  : 2e-4,
    "max_epochs"    : 100,
    ...
    # Sobremuestreo por presencia en máscara (jerarquía: GG5 > GG4 > GG3 > resto)
    "sampler_weight_gg5": 2.5,
    "sampler_weight_gg4": 1.3,
    "sampler_weight_gg3": 1.8,
    # Dropout espacial para regularizar el decoder (batch pequeño => overfitting más fácil)
    "decoder_dropout_p": 0.20,
    "use_compile"   : None,
    "conch_checkpoint": None,   # None → CONCH_HF_CHECKPOINT
    "conch_hf_token" : None,
    "seed"            : DEFAULT_SEED,
    # Máscara: gris >= gg5_gray_min → GG5 (default 170; 160 recupera LUT antigua).
    "gg5_gray_min"    : DEFAULT_GG5_GRAY_MIN,
    # Componentes de cualquier clase con menos píxeles → reasignar al vecino más frecuente (0 = desactivar limpieza).
    "mask_clean_min_area": 16,
}

# ─────────────────────────────────────────────────────────────────────────────
# Dataset  — LUT vía build_mask_lut(gg5_gray_min) + opcional clean_all_speckles (sicap_mapping)
# ─────────────────────────────────────────────────────────────────────────────
...
def compute_sample_weights(
    ...
    CE ya usa pesos por clase; estos factores solo sesgan qué tiles se ven más a menudo."""
    weights = []
    for name in image_names:
        ...
        if mask_path.exists():
            ...
            if mask is not None:
                mapped = mask_lut[mask].astype(np.int64)
                if mask_clean_min_area > 0:
                    mapped = clean_all_speckles(mapped, min_area=mask_clean_min_area)
                if np.any(mapped == 3):
                    w = weight_gg5
...
class SICAPv2Dataset(Dataset):
    def __init__(
        ...
        self.mask_lut = mask_lut if mask_lut is not None else _DEFAULT_MASK_LUT
    ...
    def __getitem__(self, idx):
        ...
        mask = self.mask_lut[mask].astype(np.int64)
        if self.mask_clean_min_area > 0:
            mask = clean_all_speckles(mask, min_area=self.mask_clean_min_area)
...
def get_fold_dataloaders(fold_name: str, config: dict):
    ...
    g5m = int(config.get("gg5_gray_min", DEFAULT_GG5_GRAY_MIN))
    mask_lut = build_mask_lut(g5m)
    clean_a = int(config.get("mask_clean_min_area", 0))
    print(
        f"  [Máscara] gg5_gray_min={g5m} (gris>={g5m}→GG5) | "
        f"clean_all_speckles: min_area={clean_a} (0=off)"
    )
    ...
    train_ds = SICAPv2Dataset(
        ...
        mask_lut=mask_lut,
        mask_clean_min_area=clean_a,
    )
    val_ds = SICAPv2Dataset(
        val_names, IMAGES_DIR, MASKS_DIR,
    ...
    if config.get("use_weighted_sampler", False):
        ...
        sw = compute_sample_weights(
            train_names, MASKS_DIR, w5, w4, w3,
            mask_lut=mask_lut,
            mask_clean_min_area=clean_a,
        )
...
class FPNDecoder(nn.Module):
    ...
    """
    def __init__(self, in_channels=768, fpn_channels=256, num_classes=4, dropout_p: float = 0.20):
        super().__init__()

        def _pick_gn_groups(num_channels: int, max_groups: int = 16) -> int:
            # Escogemos el mayor número de grupos <= max_groups que divide num_channels.
        ...
        self.lat = nn.ModuleList([
            nn.Sequential(
                ...
                nn.GroupNorm(_pick_gn_groups(fpn_channels), fpn_channels),
        ...
        self.head = nn.Sequential(
            ...
            nn.GroupNorm(_pick_gn_groups(fpn_channels // 2), fpn_channels // 2),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=dropout_p),
...
def build_model(config: dict):
    return CONCHSegModel(
        ...
        decoder_dropout_p = config.get("decoder_dropout_p", 0.20),
...
def main():
    ...
    parser.add_argument("--fold", type=str, nargs="+", default=None, choices=["Val1", "Val2", "Val3", "Val4"], help="Ejecutar solo uno o varios folds en específico (ej. Val1 Val2). Si no se indica, se ejecutan los 4.")
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="G",
        help=f"Gris mínimo para etiquetar GG5 tras LUT (default config: {DEFAULT_GG5_GRAY_MIN}; 160 = LUT histórica 160:255→GG5).",
    )
    parser.add_argument(
        "--mask-clean-min-area",
        type=int,
        default=None,
        metavar="N",
        help="Reasignar al vecino más frecuente los componentes de cualquier clase con menos de N píxeles (default config: 16; 0 = desactivar).",
    )
    ...
    if args.gg5_gray_min is not None:
        config["gg5_gray_min"] = args.gg5_gray_min
    ...
    if use_wandb and not args.dry_run:
        try:
            wandb_config = {
                ...
                "gg5_gray_min": config.get("gg5_gray_min", DEFAULT_GG5_GRAY_MIN),
                "mask_clean_min_area": config.get("mask_clean_min_area", 0),
                "mask_lut": (
                    f"43:85→GG3, 85:{config.get('gg5_gray_min', DEFAULT_GG5_GRAY_MIN)}→GG4, "
                    f"{config.get('gg5_gray_min', DEFAULT_GG5_GRAY_MIN)}:→GG5"
                ),
