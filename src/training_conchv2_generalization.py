...
Train vs val loss (same function GuidedLoss = CE ponderada + Dice):
    ...
    - Esta variante (training_conchv2_generalization.py): dropout FPN, weight_decay, label_smoothing,
      LR cosine+warmup optional, ratio encoder/decoder, EMA, augmentations HueSat+Elastic, tracking
      val_loss vs macro-F1. Checkpoints by default en checkpoints_conch_masklut_generalization/.
...
OUTPUT_DIR = default_checkpoint_dir("checkpoints_conch_masklut_generalization")
...
# ─────────────────────────────────────────────────────────────────────────────
# Augmentations — base + optional HueSat + Elastic
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directorio para checkpoints .pth (by default: checkpoints_conch_masklut_generalization).",
    )
    parser.add_argument(
        "--seed",
        type=int,
