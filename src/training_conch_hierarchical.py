...
def _extract_conch_trunk(weights_path=None, hf_token=None):
    try:
        ...
        raise ImportError("Install CONCH: pip install git+https://github.com/mahmoodlab/CONCH.git") from e

    if weights_path:
        wp = Path(weights_path)
        if not wp.is_file():
            raise FileNotFoundError(f"CONCH checkpoint not found: {wp}")
        ckpt = str(wp.resolve())
    else:
...
class ConcHEncoder(nn.Module):
    def __init__(self, feature_blocks=_FEATURE_BLOCKS, unfreeze_last=0, weights_path=None, hf_token=None):
        ...
        if unfreeze_last > 0:
            ...
            for p in self.trunk.norm.parameters():
                p.requires_grad = True
            print(f"  [ConcHEncoder] Unfroze last {unfreeze_last}/{total} blocks")
        else:
            print("  [ConcHEncoder] Frozen encoder")
    ...
    def forward(self, x):
        ...
        if h_p * w_p != num_spatial:
            raise RuntimeError(f"Invalid spatial token count: N={num_spatial}")
...
def train_fold_stage1(
    ...
    if existing_ckpts:
        best_ckpt = max(existing_ckpts, key=lambda p: float(p.stem.split('_')[3]))
        parts = best_ckpt.stem.split('_')
    ...
    if use_compile and int(torch.__version__.split('.')[0]) >= 2:
        try:
            ...
            print(f"  [WARN] torch.compile stage1 unavailable: {e}")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if macro_f1 > best["macro_f1"]:
            ...
            if patience >= patience_max:
                print(f"  [S1] Early stopping at epoch {epoch}")
                break
        if dry_run:
            break
...
def train_fold_stage2(
    ...
    if use_compile and int(torch.__version__.split('.')[0]) >= 2:
        try:
            ...
            print(f"  [WARN] torch.compile stage2 unavailable: {e}")
    elif not use_compile:
        print("  ℹ️ torch.compile disabled (recommended on Windows to save VRAM).")
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="1 epoch per stage and fold.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Threshold usado para gate de trainsmiento Stage2. Default=max(0.05, stage1-0.15).",
    )
    parser.add_argument(
        "--stage2-gate-dilation",
        type=int,
        default=None,
        help="Gate dilation kernel in Stage2 (train/val). 0/1 disables dilation.",
    )
    parser.add_argument(
        "--stage2-keep-tumor-outside-gate",
        action="store_true",
        help="Keep GT tumor pixels even if they fall outside the Stage1 gate in Stage2.",
    )
    parser.add_argument(
    ...
    if use_wandb and not args.dry_run:
        try:
            ...
            print(f"  [WARN] W&B unavailable: {e}")
            use_wandb = False
    ...
    for fold in fold_names:
        ...
        if s1["path"] is None:
            print(f"  [WARN] Fold {fold}: Stage1 sin checkpoint; no se puede trainsr Stage2 gated.")
            continue
