...
LoRA optional (--lora-rank N; default 0=disabledo) para no complicar el flujo base.
...
OUTPUT_DIR    = default_checkpoint_dir("checkpoints_conch_hybridunet")

NUM_CLASSES  = 4
CLASS_NAMES  = ["NC", "GG3", "GG4", "GG5"]
IMG_SIZE     = 512   # multiple of patch_size=16; NO subir a 1024 by default
...
# ViT-B has 12 blocks → 4 escalas equidistantes
...
DEFAULT_CONFIG = {
    ...
    # LoRA (disabledo by default para mantener velocidad de v2)
    "lora_rank"          : 0,
    "lora_alpha"         : 1.0,
    "lora_blocks"        : 4,
    # Sampler
...
def get_train_transforms():
    return A.Compose([
        ...
        # Quitada la augmentation de color lenta (HSV) porque atascaba el DataLoader.
...
# ─────────────────────────────────────────────────────────────────────────────
# LoRA — Low-Rank Adaptation (optional, disabledo by default)
...
def inject_lora(trunk, rank: int, alpha: float, lora_blocks: int = 0):
    ...
    print(f"  [LoRA] {len(lora_params)} param tensores trainsbles")
    return lora_params
...
class CONCHEncoder(nn.Module):
    ...
    Con LoRA optional en los last N bloques.
    """
    def __init__(self, feature_blocks=_FEATURE_BLOCKS, unfreeze_last=0,
        ...
        # Descongelar last bloques (complementario a LoRA)
        if unfreeze_last > 0:
    ...
    def forward(self, x):
        ...
        if H_p * W_p != num_spatial:
            raise RuntimeError(f"Tokens espaciales no square: N={num_spatial}")
...
class AttentionGate(nn.Module):
    """
    g  : señal guía (feature deepest, ya proyectada a dec_ch)
    ...
    def __init__(self, dec_ch: int):
        super().__init__()
        # Eficiencia: reducir a dec_ch//2 internamente
        inter = max(dec_ch // 2, 16)
...
class HybridUNetDecoder(nn.Module):
    ...
    Decisión de diseño: concat+conv versus sum
    ...
    def __init__(self, in_channels: int = 768, dec_ch: int = 256,
        ...
        # --- Merge: concat(dec_ch, dec_ch) → dec_ch  (más expresivo que sum) ---
        ...
        # --- Cabeza final (más expresiva que v2, lighter than UPerNet) ---
        ...
        # aux1 en features[2] (bloque 8) — señal deepest intermedia
        self.aux1 = nn.Sequential(
            nn.Conv2d(in_channels, dec_ch // 2, 3, padding=1, bias=False),
            nn.BatchNorm2d(dec_ch // 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(dec_ch // 2, num_classes, 1),
        )
        # aux2 en features[1] (bloque 5) — señal más superficial
    ...
    def forward(self, features, target_size):
        ...
        # Empezamos por la deepest (lat[3])
        x = lats[3]
        ...
        # 4. Salidas auxiliares (upsample inline, sin ramas extra en el grafo principal)
        aux1 = F.interpolate(self.aux1(features[2]), size=target_size,
                             mode="bilinear", align_corners=False)
        aux2 = F.interpolate(self.aux2(features[1]), size=target_size,
                             mode="bilinear", align_corners=False)
...
class GuidedLoss(nn.Module):
    """
    Loss base: Dice multiclass + CrossEntropy ponderada (sqrt-inv freq).
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, device,
    ...
    for images, masks in pbar:
        ...
        with autocast("cuda", enabled=(device.type == "cuda"), dtype=amp_dtype):
            ...
            # Deep supervision — only durante trainsmiento; pesos moderados
            if aux_weight_1 > 0:
...
def validate_one_epoch(model, loader, criterion, device):
    """Validación only con salida principal (las aux heads no se usan en inferencia)."""
    model.eval()
...
def train_fold(fold_name: str, config: dict, device: torch.device,
    ...
    # torch.compile — disabledo en Windows by default (igual que v2)
    ...
    if use_compile and int(torch.__version__.split('.')[0]) >= 2:
        ...
        print("  torch.compile disabledo (recomendado en Windows).")
...
def main():
    ...
    parser.add_argument("--unfreeze-last",    type=int, default=0,
                        help="Descongelar last N bloques ViT-B (0=congelado)")
    parser.add_argument("--lora-rank",        type=int, default=0,
                        help="Rango LoRA en Q,V (0=disabledo; recomendado 4 si se activa)")
    ...
    parser.add_argument("--aux-weight-1",     type=float, default=0.3,
                        help="Peso loss auxiliar features[2] (bloque 8). 0=disabler.")
    parser.add_argument("--aux-weight-2",     type=float, default=0.15,
                        help="Peso loss auxiliar features[1] (bloque 5). 0=disabler.")
