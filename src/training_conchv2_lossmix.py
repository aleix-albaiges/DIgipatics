...
Train vs val loss (LossMix = 4-class main + binary tumor + grade-only on tumor):
    - The reported loss is a weighted sum; it is not a probability.
...
# W&B project for the loss-mix branch
WANDB_PROJECT_DEFAULT = "SICAPv2_CONCH_LossMix"
...
# Checkpoints for the loss-mix branch
OUTPUT_DIR = default_checkpoint_dir("checkpoints_conch_lossmix")
...
DEFAULT_CONFIG = {
    ...
    "tumor_loss_weight": 0.25,
    "grade_loss_weight": 0.35,
    "tumor_pos_weight": 2.5,
    "grade_hard_topk": 0.70,
...
def grade_target_from_mask4(mask4: torch.Tensor) -> torch.Tensor:
    """Map NC/GG3/GG4/GG5 -> IGNORE/GG3/GG4/GG5 for tumor-only supervision."""
...
# ─────────────────────────────────────────────────────────────────────────────
# LossMix — same logits, three complementary supervision signals
# ─────────────────────────────────────────────────────────────────────────────
class LossMix(nn.Module):
    def __init__(
        self,
        class_weights: list,
        grade_class_weights: list,
        *,
        dice_weight=0.55,
        ce_weight=0.45,
        tumor_loss_weight=0.25,
        ...
        self.grade_hard_topk = float(grade_hard_topk)
    ...
    @staticmethod
    def tumor_logit_from_multiclass(logits: torch.Tensor) -> torch.Tensor:
        """Binary cancer logit consistent with the 4-class softmax partition."""
        nc_logit = logits[:, 0:1, ...]
        tumor_logit = torch.logsumexp(logits[:, 1:, ...], dim=1, keepdim=True)
        return tumor_logit - nc_logit

    def _grade_ce_ohem(self, grade_logits: torch.Tensor, grade_targets: torch.Tensor) -> torch.Tensor:
        per_pixel = self.grade_ce(grade_logits, grade_targets)
        valid_mask = grade_targets != IGNORE_INDEX
        valid_losses = per_pixel[valid_mask]
        if valid_losses.numel() == 0:
            return grade_logits.sum() * 0.0
        keep_ratio = min(max(self.grade_hard_topk, 0.05), 1.0)
        if keep_ratio < 0.999:
            k = max(1, int(valid_losses.numel() * keep_ratio))
            valid_losses = torch.topk(valid_losses, k=k, largest=True).values
        return valid_losses.mean()

    def forward(self, logits: torch.Tensor, targets: torch.Tensor):
        logits = logits.float()

        main_ce = self.ce_loss(logits, targets)
        main_dice = self.dice_loss(logits, targets)
        main_total = self.dice_weight * main_dice + self.ce_weight * main_ce

        tumor_logits = self.tumor_logit_from_multiclass(logits)
        tumor_targets = (targets > 0).unsqueeze(1).float()
        tumor_bce = F.binary_cross_entropy_with_logits(
            tumor_logits,
            tumor_targets,
            pos_weight=self.tumor_pos_weight_tensor,
        )
        tumor_dice = self.tumor_dice(tumor_logits, tumor_targets)
        tumor_total = 0.60 * tumor_bce + 0.40 * tumor_dice

        grade_logits = logits[:, 1:, ...]
        grade_targets = grade_target_from_mask4(targets)
        if torch.any(grade_targets != IGNORE_INDEX):
            grade_ce = self._grade_ce_ohem(grade_logits, grade_targets)
            grade_dice = self.grade_dice(grade_logits, grade_targets)
            grade_total = 0.60 * grade_ce + 0.40 * grade_dice
        else:
            grade_ce = grade_logits.sum() * 0.0
            grade_dice = grade_logits.sum() * 0.0
            grade_total = grade_logits.sum() * 0.0

        total = main_total + self.tumor_loss_weight * tumor_total + self.grade_loss_weight * grade_total
        parts = {
            "main_total": float(main_total.detach().item()),
            "main_ce": float(main_ce.detach().item()),
            "main_dice": float(main_dice.detach().item()),
            "tumor_total": float(tumor_total.detach().item()),
            "tumor_bce": float(tumor_bce.detach().item()),
            "tumor_dice": float(tumor_dice.detach().item()),
            "grade_total": float(grade_total.detach().item()),
            "grade_ce": float(grade_ce.detach().item()),
            "grade_dice": float(grade_dice.detach().item()),
        }
        return total, parts
...
class BinaryMetrics:
    ...
    def update_batch(self, binary_logits: torch.Tensor, binary_targets: torch.Tensor):
        probs = torch.sigmoid(binary_logits.float()).squeeze(1)
        preds = (probs >= self.threshold).long().detach().cpu().numpy()
        tgts = binary_targets.long().detach().cpu().numpy()
        mask = (tgts >= 0) & (tgts < 2)
        np.add.at(self.confusion_matrix, (tgts[mask], preds[mask]), 1)
...
class TumorGradeMetrics:
    def __init__(self):
        self.confusion_matrix = np.zeros((3, 3), dtype=np.int64)

    def update_batch(self, grade_logits: torch.Tensor, targets4: torch.Tensor):
        preds = grade_logits.argmax(dim=1).detach().cpu().numpy()
...
# Training & Val Epochs
# ─────────────────────────────────────────────────────────────────────────────
def _average_part_sums(part_sums: dict, num_batches: int) -> dict:
    if num_batches <= 0:
        return {k: 0.0 for k in part_sums}
    return {k: float(v) / float(num_batches) for k, v in part_sums.items()}
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, device, grad_accum_steps: int = 1):
    ...
    return {
        ...
        "parts": _average_part_sums(part_sums, num_batches),
    }
...
def validate_one_epoch(model, loader, criterion, device):
    ...
    for images, masks in pbar:
        ...
        tumor_logits = LossMix.tumor_logit_from_multiclass(logits)
        tumor_targets = (masks > 0).long()
        tumor_metrics.update_batch(tumor_logits, tumor_targets)
        grade_metrics.update_batch(logits[:, 1:, ...], masks)
    return {
        "loss": total_loss / max(num_batches, 1),
        "parts": _average_part_sums(part_sums, num_batches),
        "segmentation": metrics.compute(),
        "tumor": tumor_metrics.compute(),
        "grade": grade_metrics.compute(),
    }
...
def train_fold(fold_name: str, config: dict, device: torch.device, dry_run: bool = False, use_wandb: bool = True):
    ...
    criterion = LossMix(
        ...
        tumor_loss_weight=float(config.get("tumor_loss_weight", 0.25)),
        grade_loss_weight=float(config.get("grade_loss_weight", 0.35)),
        tumor_pos_weight=float(config.get("tumor_pos_weight", 2.5)),
        grade_hard_topk=float(config.get("grade_hard_topk", 1.0)),
    ).to(device)
    ...
    best_tumor_f1, best_grade_macro_f1 = 0.0, 0.0
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        val_metrics = val_res["segmentation"]
        tumor_metrics = val_res["tumor"]
        grade_metrics = val_res["grade"]
        macro_f1 = val_metrics["macro_f1"]
        scheduler.step(macro_f1)
        ratio = val_loss / (train_loss + 1e-8)
        print(f"  Train Loss: {train_loss:.4f}  |  Val Loss:   {val_loss:.4f}  |  Val/Train: {ratio:.3f}")
        print(f"  Macro F1:   {macro_f1:.4f}")
        print(f"  Aux Cancer F1: {tumor_metrics['cancer_f1']:.4f}  |  Aux Grade Macro F1: {grade_metrics['macro_f1']:.4f}")
        ...
        if use_wandb and wandb.run is not None:
            ...
            metrics_dict = {
                ...
                f"{fold_name}/aux_cancer_precision": float(tumor_metrics["cancer_precision"]),
                f"{fold_name}/aux_cancer_recall": float(tumor_metrics["cancer_recall"]),
            ...
            for key, value in train_res["parts"].items():
                metrics_dict[f"{fold_name}/train_{key}"] = float(value)
            for key, value in val_res["parts"].items():
                metrics_dict[f"{fold_name}/val_{key}"] = float(value)
            for i, name in enumerate(CLASS_NAMES):
                metrics_dict[f"{fold_name}/f1_{name}"] = float(val_metrics["f1_per_class"][i])
            for i, name in enumerate(GRADE_CLASS_NAMES):
                metrics_dict[f"{fold_name}/aux_grade_f1_{name}"] = float(grade_metrics["f1_per_class"][i])
            wandb.log(metrics_dict)

        if macro_f1 > best_macro_f1:
            best_macro_f1 = macro_f1
            best_cm = val_metrics["confusion_matrix"]
            best_tumor_f1 = float(tumor_metrics["cancer_f1"])
    ...
    return {
        ...
        "best_tumor_f1": best_tumor_f1,
        "best_grade_macro_f1": best_grade_macro_f1,
        "best_cm": best_cm,
    }
...
def main():
    ...
    parser.add_argument("--lr", type=float, default=None, help="Learning rate del decoder.")
    parser.add_argument("--weight-decay", type=float, default=None, help="Weight decay AdamW.")
    parser.add_argument("--patience", type=int, default=None, help="Early stopping patience.")
    parser.add_argument("--lr-plateau-patience", type=int, default=None, help="Patience del ReduceLROnPlateau.")
    parser.add_argument("--tumor-loss-weight", type=float, default=None, help="Peso de la pérdida auxiliar Cancer vs NC.")
    parser.add_argument("--grade-loss-weight", type=float, default=None, help="Peso de la pérdida auxiliar GG3/GG4/GG5 solo en tumor.")
    parser.add_argument("--tumor-pos-weight", type=float, default=None, help="pos_weight del BCE binario de tumor.")
    parser.add_argument("--grade-hard-topk", type=float, default=None, help="Fracción de píxeles tumorales más difíciles para CE grade-only (0-1].")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directorio para checkpoints .pth (by default: checkpoints_conch_lossmix junto al script).",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    if args.grade_hard_topk is not None:
        config["grade_hard_topk"] = float(args.grade_hard_topk)
    ...
    print(
        "  class_weights (main): "
        f"{config['class_weights']} | grade_class_weights: {config['grade_class_weights']} | "
        f"dice/ce(main)={config['dice_weight']}/{config['ce_weight']} | lr_plateau_patience={config.get('lr_plateau_patience', 3)}"
    )
    print(
        f"  lossmix: tumor_w={config.get('tumor_loss_weight')} | grade_w={config.get('grade_loss_weight')} | "
        f"tumor_pos_weight={config.get('tumor_pos_weight')} | grade_hard_topk={config.get('grade_hard_topk')}"
    )
    ...
    if use_wandb and not args.dry_run:
        try:
            wandb_config = {
                "script": "training_conchv2_lossmix",
                "img_size": IMG_SIZE,
                ...
                "tumor_loss_weight": config.get("tumor_loss_weight"),
                "grade_loss_weight": config.get("grade_loss_weight"),
                "tumor_pos_weight": config.get("tumor_pos_weight"),
                "grade_hard_topk": config.get("grade_hard_topk"),
                "unfreeze_last": config["unfreeze_last"],
            ...
            run_name = args.wandb_name or (
                f"CONCH_lossmix_bs{config['batch_size']}_ga{config.get('grad_accum_steps', 1)}"
            )
            wandb.init(
                project=args.wandb_project,
                name=run_name,
                config=wandb_config,
                tags=["mask_lut", "CONCH", "SICAPv2", "lossmix"],
            )
        except Exception as e:
