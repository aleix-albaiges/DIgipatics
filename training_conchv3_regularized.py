"""
SICAPv2 Sºntic Segmentation Training Pipeline
================================================
...
class GuidedLoss(nn.Module):
    def __init__(self, class_weights: list, dice_weight=0.5, ce_weight=0.5, smooth=1e-6, label_smoothing=0.0):
        super().__init__()
        ...
        self.ce_loss = nn.CrossEntropyLoss(weight=self.weights_tensor, label_smoothing=label_smoothing)
...
# ─────────────────────────────────────────────────────────────────────────────
def train_one_epoch(model, loader, criterion, optimizer, scaler, device, grad_accum_steps: int = 1, ema_model=None):
    model.train()
...
def train_fold(fold_name: str, config: dict, device: torch.device, dry_run: bool = False, use_wandb: bool = True):
    ...
    criterion = GuidedLoss(config["class_weights"], config["dice_weight"], config["ce_weight"], label_smoothing=float(config.get("label_smoothing", 0.0))).to(device)
    ...
    if config.get("use_ema", False):
        ...
        print(f"  [EMA] Activado con decay={ema_decay}")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        train_loss = train_one_epoch(
            ...
            grad_accum_steps=ga, ema_model=ema_model
        )
        eval_model = ema_model if ema_model is not None else model
        val_loss, val_metrics = validate_one_epoch(eval_model, val_loader, criterion, device)
        ...
        if macro_f1 > best_macro_f1:
            ...
            torch.save(eval_model.state_dict(), out_dir / f"best_{fold_name}_{macro_f1:.4f}.pth")
