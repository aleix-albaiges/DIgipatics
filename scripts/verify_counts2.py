...
from paths import IMAGES_DIR, MASKS_DIR, default_checkpoint_dir
from training_conchv2 import CONCHSegModel, _MASK_LUT, get_val_transforms

CHECKPOINT_PATH = default_checkpoint_dir("checkpoints_conch_masklut") / "best_Val1_0.6784.pth"

model = CONCHSegModel(num_classes=4, unfreeze_last=0)
model.load_state_dict(torch.load(CHECKPOINT_PATH, map_location="cpu", weights_only=True))
model.eval()

image_name = "16B0006694_Block_Region_1_21_6_xini_12239_yini_134070.jpg"
...
def calc_f1(c):
    ...
    return (2.0*tp)/(2.0*tp + fp + fn) if tp+fp+fn>0 else np.nan

f1_scores = [calc_f1(0), calc_f1(1), calc_f1(2), calc_f1(3)]
macro_f1 = np.nanmean(f1_scores)

print(f"F1 0: {f1_scores[0]}")
print(f"F1 1: {f1_scores[1]}")
print(f"F1 2: {f1_scores[2]}")
print(f"F1 3: {f1_scores[3]}")
print(f"Macro F1 calculated: {macro_f1}")
