...
nb_path = "c:/Users/Aleix/OneDrive - Universitat Politècnica de Catalunya/Escritorio/UNI/TFG/Recerca primers datasets/SicapV2/SICAPv2/analisis_fscore_checkpoints.ipynb"
...
print("Calculando cantidad de píxeles reales (Ground Truth) por imagen...")
pixel_counts = []
...
for name in tqdm(df_results['image_name']):
    ...
    # np.bincount es la forma más rápida de contar (0 a 3)
...
# Convertir a DataFrame y unir por 'image_name' para evitar duplicados si se corre varias veces
...
print("¡Columnas de píxeles añadidas al df_results original!")
print("-" * 50)

# Aplicar el filtro avanzado para encontrar imágenes GIGANTES y con mala métrica
...
print(f"Se han encontrado {len(df_interesante)} imágenes con más de 50.000 píxeles de tumor y un Macro F1 < 0.5")
display(df_interesante[['image_name', 'macro_f1', 'pixels_cancer', 'cancer_percentage', 'f1_GG3', 'f1_GG4', 'f1_GG5']].head(10))

# Descomenta la siguiente línea para visualizar directamente la que tenga MÁS tumor pero peor F1:
# plot_image_analysis(df_interesante.iloc[0]['image_name'], df_interesante.iloc[0]['macro_f1'])
"""

new_cell = nbf.v4.new_code_cell(new_code)
