...
OUTPUT_DIR = BASE_DIR / "checkpoints_conch_upernet"
...
def inject_lora(trunk, rank: int, alpha: float, lora_blocks: int = 0):
    """Inyecta LoRA en attn.qkv de los últimos `lora_blocks` bloques del ViT.
...
class ConcHEncoder(nn.Module):
    """CONCH ViT-B/16 visual trunk con LoRA opcional y unfreeze opcional."""
    ...
    def __init__(self, feature_blocks=_FEATURE_BLOCKS, unfreeze_last=0,
        ...
        # LoRA (PEFT) — solo en los últimos lora_blocks bloques
        self.lora_params = []
        if lora_rank > 0:
            self.lora_params = inject_lora(self.trunk, lora_rank, lora_alpha, lora_blocks)

        # Descongelar últimos bloques (opcional, complementario a LoRA)
        if unfreeze_last > 0:
    ...
    def forward(self, x):
        ...
        assert H_p * W_p == num_spatial, f"Tokens no cuadráticos: {num_spatial}"
...
class UPerNetDecoder(nn.Module):
    ...
    def __init__(self, in_channels=768, fpn_channels=256, num_classes=4,
        ...
        super().__init__()
        # PPM sobre la feature más profunda (bloque 11)
...
def main():
    ...
    parser.add_argument("--lora-rank", type=int, default=4,
                        help="Rango LoRA en Q,V (0=desactivar)")
    ...
    parser.add_argument("--aux-weight", type=float, default=0.4,
                        help="Peso de la loss auxiliar (0=desactivar)")
