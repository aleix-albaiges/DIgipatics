...
================================================
UNI v1 Backbone (H&E foundation, más ligero que UNI2-h)
...
Usage:
    ...
    python training_uni_v1.py --weights RUTA/pytorch_model.bin
...
OUTPUT_DIR = BASE_DIR / "checkpoints_uni_v1"

NUM_CLASSES = 4
CLASS_NAMES = ["NC", "GG3", "GG4", "GG5"]
IMG_SIZE = 512  # múltiplo de patch_size=16 (UNI v1 = ViT-L/16)

# Bloques del ViT-L (24) de los que extraemos features — mismos índices relativos que UNI2
...
DEFAULT_CONFIG = {
    ...
    # ── UNI v1: más ligero que UNI2-h; perfil portátil ~8GB ─────────
    "batch_size"    : 4,
    "grad_accum_steps": 2,
    "learning_rate" : 6e-5,
    "fpn_channels"  : 256,
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset  — idéntico al original (incluyendo LUT)
# ─────────────────────────────────────────────────────────────────────────────
...
class UNIEncoder(nn.Module):
    ...
    def __init__(self, feature_blocks=_FEATURE_BLOCKS, unfreeze_last=0, weights_path=None):
        ...
        if weights_path is not None:
            ...
            if not wp.is_file():
                raise FileNotFoundError(f"Pesos UNI no encontrados: {wp}")
        ...
        if unfreeze_last > 0:
            ...
            for p in self.vit.norm.parameters():
                p.requires_grad = True
            print(f"  [UNIEncoder] Descongelados los últimos {unfreeze_last}/{total} bloques")
        else:
            print("  [UNIEncoder] Encoder congelado (solo entrena el decoder FPN)")
...
def main():
    ...
    parser.add_argument("--unfreeze-last", type=int, default=0,
                        help="Descongelar los últimos N bloques del ViT UNI (default: 0 = frozen)")
