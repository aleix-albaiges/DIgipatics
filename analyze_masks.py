...
- **Parche (Excel)**: columnas NC, G3, G4, G5 en los .xlsx de ``partition/`` → índice
  0..3 vía ``sicap_mapping.excel_row_to_class_index`` (misma lógica que análisis / no G4C en el índice).

- **Píxel (máscara)**: gris → ``build_mask_lut(gg5_gray_min)`` + opcional
  ``clean_all_speckles`` (misma lógica que ``training_conch``).

Por defecto solo incluye ``image_name`` listados en la partición (Train/Test .xlsx).

Uso:
  python analyze_masks.py
  python analyze_masks.py --all-masks
...
PARTITION_DIR = BASE / "partition"

# Mismo default que training_conch DEFAULT_CONFIG
...
def _print_class_abs_pct(
    ...
    *,
    ...
    if not mutually_exclusive:
        print("  (Clases no excluyentes: un mismo parche puede contar en varias filas; los % no suman 100%.)\n")
...
def excel_class_distributions() -> tuple[Counter, Counter, int, int]:
    """
    Devuelve:
    - conteo por clase incluyendo **cada fila** Excel (puede repetir image_name entre folds),
    - conteo por clase con **image_name únicos** (última aparición al recorrer ficheros ordenados gana),
    - nº filas con clase None / ambigua,
...
def run(
    ...
    print(f"  clean_all_speckles min_area={mask_clean_min_area} (0 = desactivado)")
    print()
    ...
    if n_none:
        print(f"  (% y SUMA usan denominador {total_rows:,} filas; las {n_none:,} None no tienen fila de clase.)")
    print()
    _print_class_abs_pct(
        "Por fila Excel (cada aparicion en un .xlsx; puede repetir image_name entre folds):",
        per_row,
        total_rows,
    )
    print()
    ...
    if all_masks:
        ...
        label = "todas las máscaras en masks/"
    else:
        ...
        label = f"solo patches en partición ({len(allowed)} nombres únicos en .xlsx)"
        print(f"Nombres únicos en partición (Excel): {len(allowed):,}")
        print(f"Ficheros de máscara encontrados: {len(all_paths):,}")
        if missing_in_masks:
            print(
                f"Aviso: {len(missing_in_masks)} nombres en partición sin fichero en masks/ "
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Distribución Excel (parche) y máscaras (píxel)")
    ap.add_argument(
        "--all-masks",
        action="store_true",
        help="Usar todos los ficheros en masks/ (ignorar partition)",
    )
    ap.add_argument(
        "--gg5-gray-min",
        type=int,
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help=f"Área mínima componente para cualquier clase (default: {_DEFAULT_CLEAN_AREA}; 0 = sin limpieza)",
    )
    args = ap.parse_args()
