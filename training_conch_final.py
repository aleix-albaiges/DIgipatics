...
Pérdida train vs val (misma función GuidedLoss = CE ponderada + Dice):
    ...
    - Se registran época de mejor val_loss vs mejor macro-F1 (wandb + resumen al final del fold).
    - Opciones anti-sobreajuste: dropout en FPN, weight_decay, label_smoothing, LR cosine+warmup,
...
DEFAULT_CONFIG = {
    ...
    # CE con suavizado de etiquetas (0 = desactivado)
    "label_smoothing": 0.0,
    ...
    "encoder_lr_ratio": 10,
    # "plateau" (ReduceLROnPlateau sobre macro-F1) | "cosine" (warmup + cosine por época)
    "lr_schedule"   : "plateau",
    "lr_warmup_epochs": 2,
    "lr_min"        : 1e-7,
    ...
    "fpn_channels"  : 256,
    # Dropout 2D en la cabeza del FPN (0 = desactivado)
    "decoder_dropout": 0.1,
    "unfreeze_last" : 0,
    ...
    # Elastic + HueSaturation en train (desactivar con --no-strong-aug)
...
# ─────────────────────────────────────────────────────────────────────────────
# Augmentations — base + opcional elastic/HueSat (generalización)
# ─────────────────────────────────────────────────────────────────────────────
...
class FPNDecoder(nn.Module):
    ...
    def __init__(self, in_channels=768, fpn_channels=256, num_classes=4, dropout: float = 0.0):
        ...
        # Cabeza de segmentación (dropout opcional antes de la conv 1×1 final)
        head_layers = [
