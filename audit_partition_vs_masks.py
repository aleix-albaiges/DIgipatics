"""
Comprueba consistencia entre:
  - wsi_labels.xlsx (Gleason por slide)
  - Excel de partition/ (NC, G3, G4, G5 + G4C: subtipo de G4; ver sicap_mapping)
  - Máscaras en masks/ con sicap_mapping.MASK_LUT

Ejecutar: python audit_partition_vs_masks.py
"""
...
def gleason_to_wsi_group(gp: int, gs: int) -> str:
    """Grupo tipo grade-group usado antes (patch-level coarse)."""
...
def row_to_excel_class(row: tuple, idx: dict[str, int]) -> int | None:
    """Delega en sicap_mapping.excel_row_to_class_index (G4C no afecta al índice)."""
...
def main() -> None:
    ...
    print(f"Ficheros partition .xlsx: {len(xfiles)}")
    ...
    for name, (ec, row, idx) in unique_items.items():
        ...
        if maj == ec:
            agree_maj += 1
        # Presencia: tumor si hay >=1 píxel de esa clase; NC si no hay píxel G3/G4/G5
    ...
    print(f"Patches con Excel válido y máscara OK: {n:,}")
    print(f"Coinciden (mayoría máscara == Excel): {agree_maj:,} ({100.0 * agree_maj / n if n else 0:.2f}%)")
    print(
        f"Coinciden (presencia): máscara tiene >=1 píxel de la clase Excel (NC=tumor sin píxel G3/G4/G5): "
        f"{agree_presence:,} ({100.0 * agree_presence / n if n else 0:.2f}%)"
    )
    print(f"Sin fichero máscara: {missing_mask:,} | decode fallido: {bad_mask:,}")
    print("\nMatriz confusión [fila=Excel, columna=Máscara mayoritaria]")
    print("       " + "  ".join(f"{NAMES[j]:>6s}" for j in range(4)))
    for i in range(4):
        print(f"{NAMES[i]:>6s} " + "  ".join(f"{cm[i,j]:>6d}" for j in range(4)))

    # Relación wsi_labels (slide) vs distribución Excel en patches
    ...
    for g in group_order:
        ...
        if tot == 0:
            print("    (ningún patch de esos slides en partition)")
            continue
        for cl in range(4):
    ...
    if only_part:
        print(f"\nSlides con patches en partition pero sin fila en wsi_labels: {len(only_part)} (ej. {list(only_part)[:5]})")
    if only_wsi:
        print(f"Slides en wsi_labels sin ningún patch en partition: {len(only_wsi)}")
