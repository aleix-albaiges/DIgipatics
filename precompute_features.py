"""
Pre-computa las features de UNI2-h para todas las imágenes y las guarda en disco.

Ejecutar UNA SOLA VEZ antes de entrenar:
    python precompute_features.py

Las features se guardan en:
...
# ─── Paths (mismos que training_uni2.py) ──────────────────────────────────────
BASE_DIR      = Path(__file__).resolve().parent
IMAGES_DIR    = BASE_DIR / "images"
PARTITION_DIR = BASE_DIR / "partition"
CACHE_DIR     = BASE_DIR / "uni2_features"

IMG_SIZE = 504  # múltiplo de patch_size=14
...
# ─── Transform (sin augmentations, solo normalize) ───────────────────────────
...
def main():
    ...
    print(f"Total imágenes a procesar: {len(all_names)}")

    # Saltarse las que ya están cacheadas
    pending = [n for n in all_names if not (CACHE_DIR / (n + ".pt")).exists()]
    print(f"Pendientes (no cacheadas todavía): {len(pending)}")
    ...
    print(f"\n✅ Features guardadas en {CACHE_DIR}")
    # Estimar tamaño total
    total_bytes = sum(f.stat().st_size for f in CACHE_DIR.glob("*.pt"))
    print(f"   Tamaño total en disco: {total_bytes / 1e9:.2f} GB")
