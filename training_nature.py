...
OUTPUT_DIR = BASE_DIR / "checkpoints_nature"
...
# ─────────────────────────────────────────────────────────────────────────────
# Pre-computar Look-Up Table (LUT) a nivel global para evitar bugs de multiprocessing (pickling) en Windows
...
class SegmentationMetrics:
    ...
    def update_batch(self, logits: torch.Tensor, targets: torch.Tensor):
        # Mantener los tensores en la GPU para el cálculo masivo de píxeles
        preds = logits.argmax(dim=1)
        mask = (targets >= 0) & (targets < self.num_classes)
        
        # Filtrar valores usando la máscara en la GPU
        ...
        # Transferir solo la matriz 4x4 resultante a la CPU y sumarla a la acumulada
...
def validate_one_epoch(model, loader, criterion, device):
    ...
    # Verificar soporte AMP como en el entrenamiento
    amp_dtype = torch.bfloat16 if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else torch.float16
...
def main():
    set_seed(42)  # <-- Añadido para hacer el entrenamiento 100% reproducible
    ...
    # NOTA: cudnn.benchmark está desactivado por set_seed() para priorizar la reproducibilidad.
