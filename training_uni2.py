...
- Loss: CrossEntropy + Dice  (idéntico al original)
- Augmentations: idénticas al original
- Tracked Metric: Macro F-score (idéntico al original)

Diferencias respecto a training_nature.py:
  · build_model() → UNI2SegModel en lugar de TAH_UNet
  · batch micro 2 + grad_accum 2 por defecto (~8GB VRAM); --batch-size / --grad-accum
  · WeightedRandomSampler (GG5/GG4), CE GG5=18; lr 6e-5
  · torch.compile desactivado por defecto en Windows (--compile para forzar)
  · LR diferencial encoder/decoder (si hay parámetros de encoder trainables)
  · --unfreeze-last N para descongelar los últimos N bloques del ViT
  · OUTPUT_DIR → checkpoints_uni2

Todo lo demás (dataset, augmentations, loss, métricas, folds,
early stopping, torch.compile) es byte-by-byte idéntico al original.
...
Usage:
    ...
    python training_uni2.py --unfreeze-last 4  # fine-tune últimos 4 bloques
"""
...
OUTPUT_DIR = BASE_DIR / "checkpoints_uni2"   # directorio separado del original

NUM_CLASSES = 4
CLASS_NAMES = ["NC", "GG3", "GG4", "GG5"]
IMG_SIZE = 504  # Debe ser múltiplo de patch_size=14 → 36×14=504 (más cercano a 512)
...
DEFAULT_CONFIG = {
    ...
    "grad_accum_steps": 2,    # batch efectivo ≈ 4
    "learning_rate" : 6e-5,   # compensa batch efectivo un poco mayor
    "fpn_channels"  : 256,
    "unfreeze_last" : 0,
...
def compute_sample_weights(image_names: list, masks_dir: Path):
    """Imágenes con GG5 → 5×; solo GG4 → 2×; resto 1×."""
    weights = []
...
def get_train_transforms():
    return A.Compose([
        A.Resize(IMG_SIZE, IMG_SIZE),  # 512→504: múltiplo de patch_size=14
...
class UNI2Encoder(nn.Module):
    ...
    def __init__(self, feature_blocks=_FEATURE_BLOCKS, unfreeze_last=0):
        ...
        # Congelar todo el encoder por defecto
        ...
        if unfreeze_last > 0:
            ...
            for p in self.vit.norm.parameters():
                p.requires_grad = True
            print(f"  [UNI2Encoder] Descongelados los últimos {unfreeze_last}/{total} bloques")
        else:
            print("  [UNI2Encoder] Encoder completamente congelado (solo decoder entrena)")
...
def main():
    ...
    parser.add_argument("--unfreeze-last", type=int, default=0,
                        help="Descongelar los últimos N bloques del encoder UNI2 (default: 0 = frozen)")
    parser.add_argument("--batch-size", type=int, default=None, help="Micro-batch (por defecto 2).")
