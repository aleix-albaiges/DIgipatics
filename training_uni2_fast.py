...
Este script carga las features pre-cacheadas de UNI2-h desde disco
y entrena solo el decoder FPN. Al no ejecutar el encoder en cada batch,
el entrenamiento es ~100x más rápido.

Usage:
    python training_uni2_fast.py             # por defecto: batch 4 × accum 4 (~8GB)
    python training_uni2_fast.py --dry-run   # smoke test
    python training_uni2_fast.py --batch-size 8 --grad-accum 2 # si tienes VRAM de sobra
    python training_uni2_fast.py --no-weighted-sampler         # como antes
    python training_uni2_fast.py --cache-dir "D:\\cache\\uni2_features"

Features cache: por defecto ./uni2_features. Si existe
    %LOCALAPPDATA%\\SICAPv2\\uni2_features con .pt, se usa automáticamente (sin flags).
    También: set UNI2_FEATURES_DIR=... o python training_uni2_fast.py --cache-dir RUTA
    (en CMD no uses $env:...; usa %LOCALAPPDATA%\\SICAPv2\\uni2_features).
"""
...
def _default_cache_dir() -> Path:
    ...
    # Copia local típica (robocopy a AppData): evita OneDrive sin flags en la CLI
    ...
    if la:
        ...
        if local.is_dir():
            try:
                ...
                pass
    return (BASE_DIR / "uni2_features").resolve()


CACHE_DIR     = _default_cache_dir()
OUTPUT_DIR    = BASE_DIR / "checkpoints_uni2_fast"
...
# Perfil 8GB VRAM: micro-batch bajo + acumulación (batch 8×4 tensores 1536C OOM en ~8GB).
# Si ves: c10_cuda_check_implementation / CUDAEvent::record → suele ser OOM; cierra otros python en GPU.
DEFAULT_CONFIG = {
    "num_classes"   : NUM_CLASSES,
    "num_workers"   : 4,
    "batch_size"    : 4,
    "grad_accum_steps": 4,   # efectivo ≈ 16; sube batch con --batch-size 6/8 si te sobra VRAM
    "learning_rate" : 6e-5,
    "weight_decay"  : 1e-4,
    "max_epochs"    : 100,
    "patience"      : 30,
...
def compute_sample_weights(image_names: list, masks_dir: Path):
    """Imágenes con píxeles GG5 → peso 5; solo GG4 → 2; resto 1 (mismo criterio que training_pcam)."""
    weights = []
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset — carga features pre-cacheadas + máscara con augmentations
# ─────────────────────────────────────────────────────────────────────────────
class CachedFeaturesDataset(Dataset):
    ...
    Las augmentaciones geométricas se aplican tanto a la máscara
    ...
    def _load_mask(self, name):
        ...
        # Resize máscara al mismo tamaño que las features se computaron
    ...
    def _geom_augment_pair(mask_np, feat_list):
        """
        Misma geometría en máscara (H,W) y en cada feature [1,C,h,w]:
        flip horizontal, flip vertical, rotación 90°×k — mismas probabilidades
        que el pipeline anterior solo-máscara.
        """
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Micro-batch en GPU (por defecto 4). Si OOM: 2; si sobra VRAM: 6 u 8.",
    )
    parser.add_argument(
        "--grad-accum",
        type=int,
        default=None,
        metavar="K",
        help="Acumular K micro-batches antes de optimizer.step (por defecto 4).",
    )
    parser.add_argument(
        "--no-weighted-sampler",
        action="store_true",
        help="Desactiva oversampling de imágenes con GG5/GG4.",
    )
    ...
    if not CACHE_DIR.exists() or not any(CACHE_DIR.glob("*.pt")):
        print("❌ No se encontró el cache de features.")
    ...
    if "OneDrive" in str(CACHE_DIR):
        print(
            "   Nota: el cache está en OneDrive (I/O lento). Copia a disco local o usa\n"
            "   %LOCALAPPDATA%\\SICAPv2\\uni2_features — se usa solo si existe y hay .pt.",
            flush=True,
        )
    ...
    if device.type == "cuda":
        ...
        print(
            ...
            f"(cierra otros procesos Python en GPU si free < ~800 MiB)",
            flush=True,
        )
    print(
